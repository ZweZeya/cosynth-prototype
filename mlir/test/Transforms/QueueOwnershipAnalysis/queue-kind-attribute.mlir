...
// stamped with a cosynth.queue_kind attribute carrying that verdict, so a
// later selection/lowering pass can read it directly off each op instead
// of re-deriving the classification itself. Uses the same SPSC + MPMC
// shape as spsc.mlir/mpmc.mlir to also confirm two independently
...
// CHECK: cosynth.queue_push"{{.*}}cosynth.queue_kind = "SPSC"

// CHECK: cir.func @q1_consumer
// CHECK: cosynth.queue_pop"{{.*}}cosynth.queue_kind = "SPSC"

// CHECK: cir.func @q2_producer_a
// CHECK: cosynth.queue_push"{{.*}}cosynth.queue_kind = "MPSC"

// CHECK: cir.func @q2_producer_b
// CHECK: cosynth.queue_push"{{.*}}cosynth.queue_kind = "MPSC"

// CHECK: cir.func @q2_consumer
// CHECK: cosynth.queue_pop"{{.*}}cosynth.queue_kind = "MPSC"
