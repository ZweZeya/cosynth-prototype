...
struct QueueAccessInfo {
    ...
    // The actual push/pop operations belonging to this queue -- sets, not
    // lists, since the same op can be reached (and would otherwise be
    // recorded once per) multiple thread instances in recordQueueAccess.
    // classifyQueues() stamps each of these with the verdict once known.
